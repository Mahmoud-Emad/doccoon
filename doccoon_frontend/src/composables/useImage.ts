...
export function useImage() {
  ...
  function validateImage(file: File): ImageValidationResult {
    if (
      ...
      return {
        valid: false,
        error:
          "Unsupported format. Supported formats: PNG, JPG, JPEG, GIF, SVG, WebP",
      };
    }

    if (file.size > MAX_IMAGE_FILE_SIZE) {
      const sizeMB = (file.size / (1024 * 1024)).toFixed(2);
      const maxSizeMB = (MAX_IMAGE_FILE_SIZE / (1024 * 1024)).toFixed(0);
      return {
        valid: false,
   ...
   */
  function generateImageMarkdown(
    imageInfo: ImageInfo,
    options?: {
      alt?: string;
      width?: string;
   ...
   */
  async function getImageInfoFromDataUrl(
    dataUrl: string,
    filename?: string,
